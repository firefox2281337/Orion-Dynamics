...
from web.templates.nexus.autoreg.logic.handler_stubs import (
    ...
    set_correct_product_ipo, fix_columns_ipo_wa, get_planned_call_offset, conversion_date_bitrth
)
...
class RegistryProcessor:
    ...
    def __init__(self, number, file_path, correspondences, template_headers, register_type="КАСКО"):
        ...
        self.register_type = "КАСКО_ИЗ_ОСАГО_4_1"
    ...
    def run(self):
        ...
        try:
            ...
            result_data = result_data.with_columns([
                ...
                pl.lit("Банки.ру").alias("Категория партнера")])
            result_data = result_data.with_columns([
            ...
            if processed_data["Ссылка на проект"].is_not_null().all():
                current_step = steps (self, current_step, total_steps, "Генерация коротких ссылок...")
            ...
            processed_data = processed_data.drop("Ссылка на проект","Новый период страховая премия","Дополнительные сведения", "literal")
