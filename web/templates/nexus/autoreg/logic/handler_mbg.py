...
from web.templates.nexus.autoreg.logic.handler_stubs import (
    create_df, parse_and_format_date, generate_external_id, calculate_planned_call_date, set_priority, set_lead_type, set_insurance_group, 
    steps, clear_columns_dog, make_fio, set_correct_bank, set_proba_prol, clean_column, format_phone, clean_phone_column, 
    set_type_of_insurance, conversion_date_bitrth, get_client_age, set_discount_pc, set_special_offer_discount, set_region_and_filial_osago_kasko, 
    set_correct_product_ipo, final_format_ipo, set_type_of_insure, set_product, create_person_file, create_contracts_file_mbg, create_linked_leads_file, save_to_excel
)
...
class RegistryProcessor:
    ...
    def create_download_package(self, base_type, person, contracts, processed_data, no_phone_df, no_data_df):
    ...
    def run(self):
        ...
        try:
            ...
            processed_data = processed_data.with_columns([pl.when(pl.col("Филиал ВСК").str.contains("Управление")).then(pl.lit("Москва")).otherwise(pl.col("Регион")).alias("Регион")])
            processed_data = processed_data.with_columns([pl.when(pl.col("Филиал ВСК").str.contains("Управление")).then(pl.lit("ГК")).otherwise(pl.col("Филиал ВСК")).alias("Филиал ВСК")])
            ...
            processed_data = processed_data.with_columns([pl.when(pl.col("Регион").str.contains("Дирекция")).then(pl.lit("Москва")).otherwise(pl.col("Регион")).alias("Регион")])
            ...
            processed_data = processed_data.with_columns([pl.when(pl.col("Филиал ВСК") == "").then(pl.lit("ГК")).otherwise(pl.col("Филиал ВСК")).alias("Филиал ВСК")])
            processed_data = processed_data.with_columns([pl.when(pl.col("Регион") == "").then(pl.lit("Москва")).otherwise(pl.col("Регион")).alias("Регион")])

            processed_data = processed_data.with_columns(pl.col("ФИО").alias("Объект страхования"))
            processed_data = processed_data.with_columns(pl.when(pl.col("Дата рождения").is_not_null()).then(pl.col("Дата рождения").str.strptime(pl.Date, format="%a, %d %b %Y %H:%M:%S %Z", strict=False)).otherwise(None).alias("Дата рождения"))
            processed_data = processed_data.with_columns(pl.col("Дата окончания страхования").cast(pl.Date).dt.strftime("%d.%m.%Y").alias("Дата окончания страхования"))
            processed_data = processed_data.with_columns(pl.col("Дата рождения").cast(pl.Date).dt.strftime("%d.%m.%Y").alias("Дата рождения"))
            ...
            contracts = create_contracts_file_mbg(processed_data)
            print("21. Файл договоров создан")

            # Удаление лишних столбцов из лидов
            processed_data = processed_data.select([col for col in processed_data.columns if col not in {"literal"}])
            ...
            download_data = self.create_download_package(
                base_type, person, contracts, processed_data, no_phone_df, no_data_df
            )
