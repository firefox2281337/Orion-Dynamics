...
from web.templates.nexus.autoreg.logic.handler_stubs import (
    ...
    set_type_of_insurance, conversion_date_bitrth, get_client_age, set_discount_pc, set_special_offer_discount, set_region_and_filial, 
    set_correct_product_ipo, final_format_ipo, create_person_file, create_contracts_file, create_linked_leads_file, set_lead_id_by_employee, save_to_excel
)
    ...
    def run(self):
        ...
        try:
            ...
            result_data = result_data.with_columns([
                ...
                set_priority(), set_lead_type(base_type), 
                set_insurance_group(base_type), pl.lit("1.3 Пилот по Коммерческим банкам").alias("Кампания")])
            result_data = result_data.with_columns(set_lead_id_by_employee())
            ...
            no_phone_df = result_data.filter((pl.col("Основной телефон").is_null()) | (pl.col("Основной телефон") == ""))
            no_data_df = result_data.filter((pl.col("Вид страхования").is_null()) | (pl.col("Вид страхования") == ""))
            processed_data = result_data.filter(~((pl.col("Основной телефон").is_null()) | (pl.col("Основной телефон") == "") | (pl.col("Вид страхования").is_null()) | (pl.col("Вид страхования") == "")))
            ...
            contracts = contracts.with_columns(pl.lit("Проект").alias("Статус Фронт"))
            print("21. Файл договоров создан")
            
            # Удаление лишних столбцов из лидов
            processed_data = processed_data.select([col for col in processed_data.columns if col not in 
                                                    {"Вид полиса", "Шт., вероятность пролонгации", "Руб., вероятность пролонгации", "Возраст", "literal"}])
            ...
            zip_filename = f"{self.number}_{base_type}_ком_банки_пилот.zip"
