...
from web.templates.nexus.autoreg.logic.handler_stubs import (
    ...
    set_correct_product_ipo, final_format_ipo, create_person_file, create_contracts_file, create_linked_leads_file, save_to_excel,
    set_product, set_type_of_insure
)
...
class RegistryProcessor:
    ...
    def __init__(self, number, file_path, correspondences, template_headers, register_type="Ипотека"):
        ...
        self.register_type = "Ипотека_SOS"
    ...
    def run(self):
        ...
        try:
            ...
            result_data = result_data.with_columns([
                ...
                pl.lit("SOS").alias("Статус рассылки 3"), pl.lit("Кнопка SOS Ипотека филиал").alias("Кампания"),
            ...
            no_phone_df = result_data.filter(pl.col("Основной телефон").is_null() & pl.col("Основной e-mail").is_null())
            no_data_df = result_data.filter(pl.col("Дата окончания страхования").is_null())
            processed_data = result_data.filter(~((pl.col("Основной телефон").is_null() & pl.col("Основной e-mail").is_null()) | pl.col("Дата окончания страхования").is_null()))
            ...
            processed_data = processed_data.with_columns(pl.col("Дата рождения").dt.strftime("%d.%m.%Y").alias("Дата рождения"))
            processed_data = processed_data.with_columns(set_type_of_insure(base_type))
