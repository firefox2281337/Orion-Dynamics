...
from web.templates.nexus.autoreg.logic.handler_stubs import (
    ...
    create_contracts_olds_osago_wa_file, save_to_excel, set_priority_osago, set_product, set_departament, set_links_osago, 
...
from web.templates.nexus.autoreg.logic.data_processing import (
    fetch_pgsql_data, fetch_actuar2_data, process_data_operations, process_data_pgsql
)
...
class RegistryProcessor:
    ...
    def create_download_package(self, base_type, person, contracts, processed_data, no_phone_df, no_data_df, cars):
        ...
        try:
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                ...
                # Файл тачек
                cars_data = self.save_excel_to_memory([(cars, "Тачки")], "cars.xlsx")
                if cars_data:
                    zip_file.writestr(f"4 - Тачки {base_type}.xlsx", cars_data)
    ...
    def run(self):
        ...
        try:
            ...
            result_data = result_data.with_columns([
                ...
                set_links_osago(base_type), 
                pl.lit("4.1 УП_ВСК Москва АКЦ").alias("Кампания"), 
                pl.lit("Дивизион Москва").alias("Ответственное подразделение"), 
                pl.lit("Дивизион Москва").alias("Ответственный отдел"), 
                pl.lit(2).alias("Код подразделения")])
            result_data = result_data.with_columns([
                pl.when(pl.col("Дата окончания страхования").is_not_null()).then(pl.col("Дата окончания страхования")).alias("Приоритет"), 
            ...
            if processed_data["Ссылка на проект"].is_not_null().all():
                current_step = self.web_steps (current_step, total_steps, "Генерация коротких ссылок...")
                processed_data = create_short_links(processed_data)
            ...
            contracts = create_contracts_olds_osago_wa_file(processed_data)
            contracts = contracts.with_columns(pl.lit("ОСАГО Москва").alias("Ответственное подразделение"))
            ...
            processed_data = processed_data.drop("Дополнительные сведения", "literal")
            ...
            download_data = self.create_download_package(
                base_type, person, contracts, processed_data, no_phone_df, no_data_df, cars
            )
